...
function updateMarkovMatrices(type, results) {
  ...
  // Conditional probabilities P(next | current), not probability over all transitions.
  learningData[type].markovMatrix = {
    TT: (counts.TT + 1) / (counts.TT + counts.TX + 2),
    TX: (counts.TX + 1) / (counts.TT + counts.TX + 2),
    XT: (counts.XT + 1) / (counts.XT + counts.XX + 2),
    XX: (counts.XX + 1) / (counts.XT + counts.XX + 2)
  };
...
// KHỞI ĐỘNG
try { loadHistoricalPatternStats(); } catch (e) { console.error('[Startup] loadHistoricalPatternStats:', e.message); }
